...
from app.models.knowledge.stochastic_kinetic_model import \
    StochasticKineticModel
from app.models.mdd.togaf_mdd_models import EnterpriseArchitecture
from app.models.mdd.tropos_mdd_model import TroposModel
from app.models.system.organization import Organization
...
class WorldModel(BaseModel):
    ...
    business_profile: Optional[BusinessProfile] = None  # Add this field
    ...
    async def create(cls):
        try:
            ...
            if not business_profile:
                logging.error("Unable to create or retrieve a business profile")
                raise ValueError("Unable to create or retrieve a business profile")
            return cls(business_profile=business_profile)
        except Exception as e:
            logging.error(f"Error creating WorldModel: {str(e)}")
            raise

    def __init__(self, business_profile: BusinessProfile, **data):
        super().__init__(**data)
        self.business_profile = business_profile
        self.llm_manager = LLMManager()
        self.storage_backend = StorageBackend(business_name=self.business_profile.name, llm_agent=self.llm_manager)
        self.graph_database = GraphDatabase()
        self.ontology_manager = OntologyManager(self.llm_manager, self.storage_backend, self.graph_database)
